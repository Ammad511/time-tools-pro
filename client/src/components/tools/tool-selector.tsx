    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
      {tools.map((tool) => (
        <button
          ...
          className={`bg-white dark:bg-slate-800 border rounded-lg p-4 text-center hover:shadow-md transition-all flex flex-col items-center ${
            activeTool === tool.id
              ? "border-primary bg-primary-50 dark:bg-primary-900/20 text-primary"
              : "border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-300 hover:text-primary dark:hover:text-primary"
          }`}
        >
          <i className={`${tool.icon} text-xl mb-2`}></i>
